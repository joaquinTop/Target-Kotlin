...
inline fun ViewManager.appCompatEditText(theme: Int = R.style.TextStyle, init: AppCompatEditText.() -> Unit) = ankoView(::AppCompatEditText, theme, init)
...
fun checkUserFields(email: String, password: String, rePassword: String, name: String, gender: String): Exception {
    if (!android.util.Patterns.EMAIL_ADDRESS.matcher(email).find()) {
        return Exception.EMAIL_ERROR
    } else if (password.length < 8) {
        return Exception.PASSWORD_ERROR
    } else if (!password.equals(rePassword)) {
        return Exception.RE_PASSWORD_ERROR
    } else if (name.isBlank()) {
        return Exception.NAME_ERROR
    } else if (gender.isBlank()) {
        return Exception.GENDER_ERROR
    }
    return Exception.VALID_USER
}
